...
class PassSigner
{
    ...
    protected static function validateManifest($dir)
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_link($file->getPathname())) {
                throw new \Exception('Card contains a symlink, ' . $file . ', which is illegal', 508);
            }
        }
    ...
    protected static function validateDirectoryAsUnsignedRawPass($path, $forceClean = false)
    {
        ...
        if (file_exists("$path/manifest.json") || file_exists("$path/signature")) {
            throw new \Exception(
                $path . ' contains pass signing artifacts that need to be removed before signing.',
                510
            );
        }
    }
    ...
    protected static function checkAppleCertificate()
    {
        if (!defined('APPLE_CERTIFICATE')) {
            throw new \Exception('The location of the Apple WWDRCA certificate is not defined', 600);
        }
        if (!is_readable(APPLE_CERTIFICATE)) {
            throw new \Exception('The Apple WWDRCA certificate file ' . APPLE_CERTIFICATE . ' does not exist', 602);
        }
    }
    /**
